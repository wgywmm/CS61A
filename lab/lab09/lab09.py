def convert_link(link):
    ...
    """
    "*** YOUR CODE HERE ***"
    result = []  # 创建一个空列表用于存储链表元素
    while link is not Link.empty:
        result.append(link.first)  # 将当前节点的值添加到列表中
...
def every_other(s):
    ...
    if s == Link.empty or s.rest == Link.empty:
        return  # 链表为空或只有一个元素时无需删除

    s.rest = s.rest.rest  # 删除第一个元素后，将当前节点的next指针指向下下个节点
    every_other(s.rest)  # 递归调用，处理剩余的节点
